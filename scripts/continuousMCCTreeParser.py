...
def parseContinuousMCCTree(parsed_tree, date, location):
    clades = []
    counter = 1
    ...
    for clade in parsed_tree.find_clades():
        ...
        if len(location) == 2:
            end_latitude, end_longitude = parseCoordinatesFromTwo(details, location)
        if len(location) == 1:
            end_latitude, end_longitude = parseCoordinatesFromOne(details, location)
