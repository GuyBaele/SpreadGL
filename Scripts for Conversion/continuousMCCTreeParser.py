...
def parseContinuousMCCTree(parsed_tree, date_format, coordinate_annotation):
    clades = []
    counter = 1
    ...
    for clade in parsed_tree.find_clades():
        ...
        if coordinate_annotation == 'two':
            end_latitude, end_longitude = parseCoordinatesFromTwo(details)
        if coordinate_annotation == 'one':
            end_latitude, end_longitude = parseCoordinatesFromOne(details)
